...
class DragSortViewState extends State<DragSortView>
  ...
  void _init(BuildContext context, EdgeInsets padding, EdgeInsets margin) {
    ...
    _itemWidth = (width - space * 2) / _itemRowCount;
    _positions.clear();
    for (int i = 0; i <= _itemCount; i++) {
      double left = (space + _itemWidth) * (i % _itemRowCount);
      double top = (space + _itemWidth) * (i ~/ _itemRowCount);
  ...
  int _checkIndexTop(Rect other) {
    int index = -1;
    double? area;
    for (int i = 0; (i < _itemRowCount && i < _itemCount); i++) {
      Rect rect = _positions[i];
  ...
  int _checkIndexBottom(Rect other) {
    ...
    if (tagIndex != -1) {
      for (int i = _itemCount - 1; i >= 0; i--) {
        if (((i + 1) / _itemRowCount).ceil() >=
                (((_dragIndex + 1) / _itemRowCount).ceil()) &&
            (i % _itemRowCount == tagIndex)) {
          return i;
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    int column = (_itemCount > _itemRowCount ? _itemRowCount : _itemCount + 1);
    int row = _itemCount ~/ 2 + 1;
    double realWidth =
    ...
    return GestureDetector(
      onLongPressStart: (LongPressStartDetails details) {
        /// find the stack offset in x and y
        Offset offset = _getWidgetLocalToGlobal(context);

        /// get the index of the drag
        /// details.localPosition: local offset including padding and margin
        /// left: margin|padding left
      ...
      onLongPressMoveUpdate: (LongPressMoveUpdateDetails details) {
        ...
        /// global position: get cursor position - old cursor position
        /// dx + original offset
